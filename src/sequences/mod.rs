...
impl<'a, T> Choose<&'a T> for &'a [T] {
    fn choose<R: RngCore + ?Sized>(self, rng: &mut R) -> Option<&'a T> {
        if self.is_empty() {
            None
        } else {
...
impl<'a, T> Choose<&'a mut T> for &'a mut [T] {
    fn choose<R: RngCore + ?Sized>(self, rng: &mut R) -> Option<&'a mut T> {
        if self.is_empty() {
            None
        } else {
            let len = self.len();
...
impl<T> Choose<T> for Vec<T> {
    fn choose<R: RngCore + ?Sized>(mut self, rng: &mut R) -> Option<T> {
        if self.is_empty() {
            None
        } else {
...
pub trait Shuffle {
    ...
    /// println!("{:?}", y);
    /// ```
    fn shuffle<R: RngCore + ?Sized>(self, rng: &mut R);
}
